...
const generateProfilePicture = async (buffer, size) => {
	...
	const jimp = await readFn(buffer);
	const w = typeof jimp.getWidth === 'function' ? jimp.getWidth() : jimp.width;
	const h = typeof jimp.getHeight === 'function' ? jimp.getHeight() : jimp.height;
	let cropped;
	if (size) {
		const min = Math.min(w, h);
		const x = (w - min) / 2;
		const y = (h - min) / 2;
		if (typeof jimp.crop === 'function' && jimp.crop.length >= 4) {
			cropped = jimp.crop(x, y, min, min);
		} else if (typeof jimp.crop === 'function') {
			try {
				cropped = jimp.crop(x, y, min, min);
			} catch {
				cropped = jimp.crop({ x, y, w: min, h: min });
			}
		} else {
			cropped = jimp.crop({ x, y, w: min, h: min });
		}
		if (cropped && cropped.resize) {
			cropped = cropped.resize.length >= 2 ? cropped.resize(size, size) : cropped.resize({ w: size, h: size });
		}
	} else {
		cropped = typeof jimp.crop === 'function' ? (jimp.crop.length >= 4 ? jimp.crop(0, 0, w, h) : jimp.crop({ x: 0, y: 0, w, h })) : jimp;
	}
	const mimeJpeg = Jimp.MIME_JPEG || jimpPkg.JimpMime?.jpeg || 'image/jpeg';
	let outBuf;
	if (typeof cropped.scaleToFit === 'function') {
		outBuf = await cropped.scaleToFit(720, 720).getBufferAsync(mimeJpeg);
	} else if (typeof cropped.getBufferAsync === 'function') {
		outBuf = await cropped.getBufferAsync(mimeJpeg);
	} else if (typeof cropped.getBuffer === 'function') {
		outBuf = await cropped.getBuffer(mimeJpeg);
	}
	return {
		img: outBuf,
		preview: outBuf
	...
	return new Promise((resolve, reject) => {
		if (typeof path === 'string' && /http/.test(path)) {
			axios.get(path).then((res) => {
				...
				if(!isNaN(length)) resolve(bytesToSize(length, 2))
			})
...
// Normalisasi jawaban game (audit fix #7): lowercase, trim, unicode normalize,
// hapus simbol, hapus spasi ganda, hapus karakter yang tidak penting.
// Dipakai supaya perbandingan jawaban lebih fleksibel/toleran terhadap
// variasi penulisan (contoh: "U.S.A", "usa", "  USA  " dianggap sama).
function normalizeAnswer(text) {
	...
	return String(text)
		...
		.toLowerCase()
		.replace(/[^\p{L}\p{N}\s]/gu, '') // hapus simbol/tanda baca
		.trim()
...
function assertInstalled(cmd, name, code) {
	try {
		...
		console.warn(chalk.yellowBright(`⚠️  ${name} is not installed or not in PATH.`));
		console.warn(chalk.gray(`[SYSTEM] Continuing execution... (Some media conversion features requiring ${name} might fail until installed).`));
		if (process.env.STRICT_DEPENDENCY_CHECK === 'true') {
			process.exit(code);
		}
	}
}
