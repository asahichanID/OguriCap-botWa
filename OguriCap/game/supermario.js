...
async function kirimSuperMario(conn, chatId, customHtml, title = '🍄 Super Mario Mini') {
...
async function handler(m, options = {}) {
    ...
    try {
        await kirimSuperMario(sock, chatId);
    } catch (err) {
...
export { pluginConfig as config, handler, kirimSuperMario, html as SUPERMARIO_HTML };
