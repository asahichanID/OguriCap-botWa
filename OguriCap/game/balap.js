...
export async function kirimBalap(conn, chatId, html = BALAP_HTML, judul = "🏎️ Turbo Race 2.5D") {
  return kirimForwardSigned(conn, chatId, html, judul);
}
...
async function handler(m, options = {}) {
  const client = options?.sock || options?.conn || options?.naze || options;
  const chatId = m?.chat || m?.key?.remoteJid;
  try {
    await kirimBalap(client, chatId);
  } catch (err) {
    console.error("[BALAP ERROR]", err);
    if (typeof m?.reply === "function") {
      await m.reply("❌ Gagal mengirim game: " + (err?.message || err));
