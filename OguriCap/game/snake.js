...
async function kirimSnake(conn, chatId, customHtml, title = '🐍 Ular Rimba') {
...
const handler = async (m, options = {}) => {
  ...
  try {
    await kirimSnake(sock, chatId);
  } catch (e) {
    console.error('[SNAKE ERROR]', e);
    if (typeof m?.reply === 'function') {
      await m.reply('❌ Gagal mengirim game Snake: ' + (e?.message || e));
    }
  }
};

export { pluginConfig as config, handler, kirimSnake, html as SNAKE_HTML };
