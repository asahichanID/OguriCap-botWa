...
<div id="chess-app">
  ...
  <div class="chess-info" id="chess-info">Pilih mode permainan.</div>
  <div class="chess-controls">
    <select id="chess-mode" onchange="resetChessGame()">
      <option value="bot">Lawan Bot</option>
      <option value="two">2 Pemain</option>
          </select>
    <select id="chess-side" onchange="resetChessGame()">
      <option value="w">Kamu Putih</option>
      <option value="b">Kamu Hitam</option>
    </select>
  </div>
...
var chessSelected=null;
  ...
  var pieces={r:'♜',n:'♞',b:'♝',q:'♛',k:'♚',p:'♟',R:'♖',N:'♘',B:'♗',K:'♔',Q:'♕',P:'♙'};
  ...
  window.chessTap=function(r,c){
    ...
    if(!legal){sChessIllegal(); var info=id('chess-info'); if(info)info.textContent='Gerakan tidak boleh. Ikuti aturan bidak dan jangan biarkan raja skak.'; return;}
  ...
  function updateChessInfo(){
    ...
    if(!moves.length){if(!chessGameOver&&!chessWinPlayed){if(check)sChessWin();else sChessDraw();chessWinPlayed=true;} chessGameOver=true; info.textContent=check?('Skakmat! '+(chessTurn==='w'?'Hitam':'Putih')+' menang.'):('Seri! Tidak ada langkah aman.'); renderChess(); return;}
    if(check)sChessCheck();
    var modeText=chessMode==='two'?'Mode 2 pemain.':'Mode lawan bot.';
    info.textContent=modeText+' Giliran '+(chessTurn==='w'?'Putih':'Hitam')+(check?' · RAJA SEDANG SKAK!':'');
  }
...
export { kirimCatur, pluginConfig as config, handler };
