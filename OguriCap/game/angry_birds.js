...
async function kirimAngryBirds(conn, chatId, customHtml, title = "🐦 Angry Birds Mini") {
...
async function handler(m, options = {}) {
    ...
    try {
        await kirimAngryBirds(sock, chatId);
    } catch (err) {
        console.error('[ANGRYBIRD ERROR]', err);
        if (typeof m?.reply === 'function') {
            await m.reply('❌ Gagal mengirim game Angry Birds: ' + (err?.message || err));
        }
    }
}

export { pluginConfig as config, handler, kirimAngryBirds, html as ANGRY_BIRDS_HTML };
